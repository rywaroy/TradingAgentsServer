...
function wrapAxiosError(error: unknown, tsCode: string, startDate: string, endDate: string): Error {
  if (error instanceof Error && (error as AxiosError).isAxiosError) {
    const axiosError = error as AxiosError;
    const status = axiosError.response?.status;
    const statusText = axiosError.response?.statusText;
    return new Error(
      `Tushare 请求异常 ts_code=${tsCode} start=${startDate} end=${endDate} status=${status} ${statusText ?? ''} ${axiosError.message}`
    );
  }
  if (error instanceof Error) {
    return new Error(`Tushare 请求异常 ts_code=${tsCode} start=${startDate} end=${endDate}: ${error.message}`);
  }
  return new Error(`Tushare 请求异常 ts_code=${tsCode} start=${startDate} end=${endDate}`);
}
